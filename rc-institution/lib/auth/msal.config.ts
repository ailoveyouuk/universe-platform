...
// learner app, so institution contacts aren't limited to having a
// Microsoft/workforce account we've added as a guest in our own tenant.
// This is a build-time switch, not a runtime one: rc-institution is a
...
// useInstitutionAuth's reactive InteractionRequiredAuthError handling. This runs
...
const OWN_KEYS = new Set(['rc-institution:selectedInstitutionId', CACHE_STAMP_KEY])
...
export function clearStaleAuthCacheIfNeeded(): void {
  ...
  try {
    ...
    // fallback in useInstitutionAuth still covers the stale-account case.
  }
}
