...
import { msalConfig, apiRequest, clearStaleAuthCacheIfNeeded } from './msal.config'
...
export function MsalProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    // Without this, a previous broken loginRedirect can leave sessionStorage in
    // a state that makes every subsequent load appear stuck.
    try {
    ...
    Promise.race([
      ...
      .catch((err: unknown) => {
        ...
        // On timeout, still attempt to mount — better to show the sign-in
        // button than hang forever. Auth will work on redirect back.
  ...
  if (!instance) {
    return (
      <div className="flex h-screen items-center justify-center bg-emp-bg">
        <div className="w-6 h-6 border-2 border-emp-accent border-t-transparent rounded-full animate-spin" />
      </div>
    )
  }
